...
case 'orderList':
	...
	$allowed_order_columns = ['trade_no', 'out_trade_no', 'name', 'buyer', 'api_trade_no'];
	$sql=" 1=1";
	$params = [];
	...
	if(!empty($_POST['starttime']) || !empty($_POST['endtime'])){
		...
		if(!empty($_POST['endtime'])){
			$endtime = preg_replace('/[^0-9\-]/', '', $_POST['endtime']);
			$sql.=" AND A.addtime<=:endtime";
			$params[':endtime'] = $endtime.' 23:59:59';
		}
	}
	if(isset($_POST['value']) && !empty($_POST['value']) && isset($_POST['column'])) {
		$col = $_POST['column'];
		if(in_array($col, $allowed_order_columns)){
			if($col=='name'){
				$sql.=" AND A.`{$col}` like :search_value";
				$params[':search_value'] = '%'.$_POST['value'].'%';
			}else{
				$sql.=" AND A.`{$col}`=:search_value";
				$params[':search_value'] = $_POST['value'];
			}
		}
	}
...
case 'riskList':
	$allowed_risk_columns = ['trade_no', 'out_trade_no', 'uid', 'ip'];
	$sql=" 1=1";
	$params = [];
	if(isset($_POST['value']) && !empty($_POST['value']) && isset($_POST['column'])) {
		$col = $_POST['column'];
		if(in_array($col, $allowed_risk_columns)){
			$sql.=" AND `{$col}`=:search_value";
			$params[':search_value'] = $_POST['value'];
		}
	}
...
case 'order': //订单详情
	$trade_no=trim($_GET['trade_no']);
	$row=$DB->getRow("select A.*,B.showname typename,C.name channelname from pre_order A,pre_type B,pre_channel C where A.trade_no=:trade_no and A.type=B.id and A.channel=C.id limit 1", [':trade_no'=>$trade_no]);
	if(!$row)
...
case 'fillorder': //手动补单
	...
	if($DB->exec("update `pre_order` set `status` ='1' where `trade_no`=:trade_no", [':trade_no'=>$trade_no])){
		$DB->exec("update `pre_order` set `endtime` =:date,`date` =NOW() where `trade_no`=:trade_no", [':date'=>$date, ':trade_no'=>$trade_no]);
